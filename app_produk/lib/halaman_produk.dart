...
class _HalamanProdukState extends State<HalamanProduk> {
  ...
  Future<void> _getdata() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.10.100/api_produk/read.php'));
  ...
  Future<bool> _hapus(String idProduk) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.10.100/api_produk/delete.php'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        backgroundColor: Colors.pinkAccent, // Ubah warna latar belakang tombol
        onPressed: () {
          print('FloatingActionButton clicked'); // Debugging log
